...
function Info() {
  ...
  const [generalPlant, setGeneralPlant] = useState<Omit<
    GeneralPlant,
    'plants'
  > | null>(null);
  ...
  useEffect(() => {
    if (plant) {
      axios
        .get(API_URL + GENERAL_PLANTS_URL + '/' + plant.generalPlantId, {
          headers: { Authorization: `Bearer ${Cookies.get('token')}` },
        })
        .then((response) => {
    ...
    <div className="h-full w-full">
      ...
      <div className="flex flex-col my-8 mx-12">
        <div className="flex flex-row mb-8">
          <img
            src={require('../figures/' + generalPlant?.image)}
            className="mr-12 border-2 rounded-md"
            alt={generalPlant?.type}
            width={250}
            height={250}
          />
          <PlantInformation
            name={plant?.name}
            type={generalPlant?.type}
            humidity={plant?.sensor?.humidity}
            light={plant?.sensor?.light}
            temperature={
              plant?.sensor?.temperature != null
                ? String(plant?.sensor?.temperature)
                : null
            }
            humidityMin={generalPlant?.humidityMin}
            humidityMax={generalPlant?.humidityMax}
            lightMin={generalPlant?.lightMin}
            lightMax={generalPlant?.lightMax}
        ...
        <div className="text-2xl italic" style={{ fontFamily: 'judson' }}>
          <div className="flex flex-row items-center mb-4">
            ...
            <p className="text-gray-500">
              {plant?.sensor?.name} ({plant?.sensor?.id})
            </p>
          </div>
