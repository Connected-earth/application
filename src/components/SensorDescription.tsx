      ...
      <div className="flex flex-col m-8 w-full justify-between">
        <div className="flex flex-row items-center justify-between">
          ...
          <div className="flex flex-row items-center text-3xl ml-auto">
            ...
            <div>
              <DeleteButton
                endpoint={API_URL + SENSOR_URL + '/' + sensor.info.sensorId}
              />
            </div>
          </div>
        </div>
        <p className="text-lg text-left my-3 italic text-gray-500">
          {sensor.info.sensorRemark}
        </p>
        <p className="text-lg text-left my-3">
          Assigned to plant:{' '}
          {sensor.info.plantName ? (
            <Link
              className={'link-hover'}
              to="/Info"
              state={{ plantId: sensor.info.plantId }}
            >
              {sensor.info.plantName}{' '}
            </Link>
          ) : (
